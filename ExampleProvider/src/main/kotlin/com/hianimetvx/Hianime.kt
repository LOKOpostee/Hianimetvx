...
class Hianime : MainAPI() {
    ...
    override suspend fun getMainPage(): HomePageResponse {
        val doc = app.get(baseUrl, interceptor = CloudflareKiller()).document
        val latest = doc.select("div.latest-episodes div.item, div.recent div.card, div.anime-card, div.item, div.anime").map { el ->
            val title = el.select("a.title, h3 a, .name, .title").text().trim()
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        val doc = app.get("$baseUrl/search?keyword=$query", interceptor = CloudflareKiller()).document
        return doc.select("div.search-item, div.anime-card, div.item").map { el ->
            AnimeSearchResponse(el.select("h3 a, .title").text(), fixUrl(el.select("a").attr("href")), name, TvType.Anime, fixUrl(el.select("img").attr("src") ?: el.select("img").attr("data-src")))
        }
    }
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val poster = fixUrl(doc.select("div.poster img, img.poster").attr("src"))
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit) {
        ...
        val iframe = doc.select("iframe[src*='player'], iframe[src*='embed']").attr("src")
